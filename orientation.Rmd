...
Tinnitus is a binary trait, becasue the individual either has it or they don't 
**Q3:** Recode f.4803
...
By recoding these fields we will have a more direct over view of the number of patients' tinnitus changed over time.In this case, if the patient answer yes they they are counted as having tinnitus, but if they answer "No" they are counted as not having tinnitus. If they did not answer or wish to not answer the question, their data is not accounter for.
...
Base on the definition of case(there is a in in any of the f.4803_recode) there are a total of 43206 cases, I am assuming the control is the False answser, so there are a total of 101550 control
...
Because the have been asked the question mutiple times, and each time they were asked the question, they are at different age.

**Q7:** For those with more than one age records, which age do you think should be used in the genetic association analysis?
depends on which period's data we are using , and we should use the data from the corresponing date. 
...
summary(fit_1)
```
Yes there is a positive correlation between `mpg of the car` and `weight of the car`. It is not that significant since the adjusted square is less than 0.7.
...
The smaller cars are more likely to have an mpg greater than 23
```{r}
auto_data <- ISLR::Auto
mpg23 <- ifelse(auto_data$mpg > 25,1,0)
fit_2<-glm(mpg23 ~ auto_data$weight)
summary(fit_2)
...
Base on the Estimates Weight has a -3.94 effect on the mpg of the car.
